...
public class databaseHelper {
    ...
    public ResultSet getModels(String brand) throws SQLException {
       try {
            PreparedStatement predStatement =  conn.prepareStatement("SELECT model FROM models WHERE brand = ?;");
            predStatement.setString(1, brand);
    ...
    public ResultSet getFuelTypes() throws SQLException {
        try {
            PreparedStatement predStatement =  conn.prepareStatement("SELECT * FROM fuelTypes;");
            ResultSet resultset = predStatement.executeQuery();
            return resultset;
        } catch (SQLException ex) {
            //Logger.getLogger(databaseHelper.class.getName()).log(Level.ERROR, null, ex);
